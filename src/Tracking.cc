...
void Tracking::GrabImage(const sensor_msgs::ImageConstPtr& msg) {
    ...
    if (mState == eTrackingState::WORKING || mState == eTrackingState::LOST)
    ...
    if (mState == eTrackingState::NOT_INITIALIZED) {
        ...
        if (mState == eTrackingState::WORKING && !RelocalisationRequested()) {
            if (!mbMotionModel || mpMap->KeyFramesInMap() < 4 || mVelocity.empty() ||
...
void Tracking::FirstInitialization() {
    ...
    if (mCurrentFrame.mvKeys.size() > 100) {
        ...
        mState = eTrackingState::INITIALIZING;
    }
}
...
bool Tracking::Relocalisation() {
    ...
    {
        std::scoped_lock lock(mMutexForceRelocalisation);
        mbForceRelocalisation = false;
...
void Tracking::Reset() {
    {
        std::scoped_lock lock(mMutexReset);
        mbPublisherStopped = false;
        mbReseting = true;
    }
